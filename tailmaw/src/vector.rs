...
impl<const N: usize, T: Copy> X<T> for Vector<N, T> {
    const VALID: () = if N < 1 {panic!("Vector has too few elements")};
    fn x(&self) -> T {
...
impl<const N: usize, T: Copy> Y<T> for Vector<N, T> {
    const VALID: () = if N < 2 {panic!("Vector has too few elements")};
    fn y(&self) -> T {
...
impl<const N: usize, T: Copy> Z<T> for Vector<N, T> {
    const VALID: () = if N < 3 {panic!("Vector has too few elements")};
    fn z(&self) -> T {
...
impl<const N: usize, T: Copy> W<T> for Vector<N, T> {
    const VALID: () = if N < 4 {panic!("Vector has too few elements")};
    fn w(&self) -> T {
        let _ = <Self as Z<T>>::VALID;
        self.values[3]
    }
...
impl<const N: usize, T: Mul<Output = T> + Copy + Default> Mul<T>
for Vector<N, T>
{
    type Output = Vector<N, T>;
...
impl<const N: usize, T: Mul<Output = T> + Copy + Default> MulAssign<T>
for Vector<N, T>{
